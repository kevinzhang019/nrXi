...
Watchers spawn at `gameDate − 6h` (`PRE_GAME_LEAD_MS = 6h`). Day games (start ≤ 18:00 UTC) hit `Math.max(now, gameDate - 6h)` resolving to supervisor wake and start watching immediately; night games sit in `setTimeout` (zero resource cost — no Redis, no MLB calls, no lock) until their slot arrives. Pre-game cadence has two modes keyed off the hoisted `pregameComputeDone` flag in `services/run-watcher.ts`. **Before the first preview compute** (lineups + probable pitchers haven't posted yet), pre-game ticks fire on a ramp — 30-min cadence until T-30m, 5-min in T-30m→T-5m, 1-min in the final 5 min — so the watcher catches the moment lineups post and fires the first compute. **Once both lineups post and probable pitchers are known**, the watcher runs the same Phase 1 + Phase 2 pipeline as live, surfaces a first-inning prediction on the dashboard, and then **goes IDLE** — one long sleep until ~T-35min from scheduled first pitch (capped at 4h per sleep). After that wake, cadence drops to 60s, then **30s once MLB flips `detailedState` to "Warmup"** (typically ~30 min before first pitch). The Warmup fast-poll catches the Warmup→Live transition within 30s. The post-compute idle is the explicit "worker idle between first compute and warmup" behavior — a late lineup scratch in that window isn't reflected on the dashboard until the T-35min wake, and the Warmup-window poll catches it in time for the first live tick. Persistence to Supabase is still gated on `status === "Live"` (Warmup classifies as "Pre"), so `inning_predictions` rows only land at first pitch. Watch for `watcher / pregame-compute-done` in Railway logs to confirm the watcher entered idle mode; `watcher / active-start` confirms the active-phase lifecycle started at Warmup or Live.
...
- **After lineups post (~30 min before first pitch typically):** `{status: "Pre", pNoHitEvent: 0.6x, pitcher: <id>, lineups: true}`. The dashboard's Upcoming card shows a real `P(nr1i) / <odds>` pill, both lineup columns are populated, and the pitcher row carries the probable starter. Any lineup/pitcher change triggers a Phase 1 reload on the next tick and a fresh prediction — caught within ≤30s once the game is in Warmup.
- **During Warmup (`detailedState: "Warmup"`):** still `{status: "Pre", ...}` but ticks now fire every 30s. Confirm in Railway logs by watching `step.fetchLiveDiff:ok` lines for the gamePk — the interval should tighten to ~30s well before first pitch.
