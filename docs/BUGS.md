...
2. **`MAX_RUNTIME_MS`** (the 6h wall-clock cap *at the time* — bumped to 20h when pre-game compute landed, then split in 2026-05 into a two-phase budget: `MAX_PREGAME_MS = 12h` from spawn while `activeStartedAt === null`, then `MAX_RUNTIME_MS = 16h` from the hoisted `activeStartedAt` once the first Warmup or Live tick arrives; see [ARCHITECTURE.md](ARCHITECTURE.md) for current rationale) — same shape: `log.warn(...); return { reason: "max-runtime" };`.
...
`MAX_LOOPS` was also bumped from 1500 to 5000 (≈7h at 5s/loop) so normal games never hit the budget. The 6h wall-clock `MAX_RUNTIME_MS` was the real ceiling at the time of this fix; MAX_LOOPS is just a defense against tight-loop bugs. (The wall-clock cap was later bumped to 20h when pre-game compute landed under `PRE_GAME_LEAD_MS = 6h`, then split in 2026-05 into a two-phase budget — `MAX_PREGAME_MS = 12h` from spawn + `MAX_RUNTIME_MS = 16h` from the first Warmup/Live tick — so pre-game duration never erodes the game-phase budget. `MAX_LOOPS` is now 20000 as a single total counter. See [ARCHITECTURE.md](ARCHITECTURE.md) for the current rationale.)
...
- The 60s `DEFAULT_STALE_AFTER_MS`. Raising it delays cleanup; lowering it risks racing the publish/exit gap inside a single tick. Don't try to use it to cover Delayed (60s polling) or Suspended (300s polling) games — those still have a live lock and the lock check filters them out before the threshold matters.
...
- Long games (extras + delays, doubleheaders near 6h after the first under the *then-current* 6h `MAX_RUNTIME_MS` — now a two-phase budget with `MAX_PREGAME_MS = 12h` from spawn + `MAX_RUNTIME_MS = 16h` from the first Warmup/Live tick), crashes (`error`), container evictions / SIGTERM (`abort`), supervisor restarts → `abandoned` → captures lost.
