...
1. `services/supervisor.ts` schedules each watcher at `gameDate − 6h` (`PRE_GAME_LEAD_MS = 6h`) — day games spawn at supervisor wake, night games wait in `setTimeout` (zero resource cost) until their window opens. Each spawned watcher polls MLB on a ramped pre-game cadence while `status === "Pre"` and the preview hasn't been computed yet — 30 min until T-30m, 5 min in T-30m→T-5m, 1 min in the final 5 min — so the watcher catches the moment lineups + probable pitchers post and fires the first preview compute. After that compute lands on the dashboard, the watcher goes IDLE in one long sleep until ~T-35min from scheduled first pitch (capped at 4h), then resumes at 60s polling, then 30s once MLB flips `detailedState` to "Warmup" (~30 min before first pitch) so the Warmup→Live transition surfaces quickly. The 6h window is the conservative tail of when MLB actually publishes lineups (typically T-1h to T-3h) and locks probable pitchers (usually by T-6h); going wider would just burn lock-refresh + feed-poll cost for no signal.
